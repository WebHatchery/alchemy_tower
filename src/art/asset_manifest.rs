...
use super::asset_keys::{
    ...
    FEMALE_PLAYER_PORTRAIT_ID, ITEM_ICON_CATEGORY, JOURNAL_TAB_CATEGORY, PLAYER_ID,
    PLAYER_PORTRAIT_ID, STATION_CATEGORY, TITLE_SCREEN_CATEGORY, TOAST_ICON_CATEGORY,
    WORLD_NODE_CATEGORY,
};
...
fn add_characters(configs: &mut Vec<TextureConfig>, data: &GameData) {
    let mut character_ids = vec![
        PLAYER_ID.to_owned(),
        FEMALE_PLAYER_ID.to_owned(),
        PLAYER_PORTRAIT_ID.to_owned(),
        FEMALE_PLAYER_PORTRAIT_ID.to_owned(),
    ];
