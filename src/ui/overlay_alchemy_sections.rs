...
use crate::alchemy_layout::{
    material_next_rect, material_previous_rect, material_row_rect_at, AL_LW, AL_LX, AL_MAT_BOX_H,
    AL_MAT_BOX_Y, AL_MAT_TITLE_Y, AL_MAT_VISIBLE_ROWS,
};
...
pub(crate) fn draw_alchemy_materials_panel_view(
    ...
    let sort_label = if view.range_text.is_some() {
        String::new()
    } else {
    ...
    if view.range_text.is_some() {
        crate::ui::draw_action_button(material_previous_rect(), view.previous_label, 0.0);
        crate::ui::draw_action_button(material_next_rect(), view.next_label, 0.0);
    }
